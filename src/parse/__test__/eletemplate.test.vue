<template>
  <div class="city_container">
    <div>
      <el-table-column prop="pcCover" label="pc封面" width="200" v-if="whatever">
        <template slot-scope="scope">
          <picture>
            <source
              :srcset="`${qiniuUpload(scope.row.cov, 2, 200, 0)}/format/webp`"
              type="image/webp"
            />
            <img
              class="avatar"
              alt="cover"
              :src="`${qiniuUpload(scope.row.cov, 2, 200, 0)}/format/gif`"
            />
          </picture>
        </template>
      </el-table-column>
      <comp v-else-if="whatever === 3">
        {{gggg}}
      </comp>
    </div>
